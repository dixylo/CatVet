...
{
    /// <summary>
    /// Interacts with users about the invoice
    /// </summary>
    public partial class InvoiceForm : Form
    {
        ...
        private void printInvoices_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            linesSoFarHeading++;
            g.DrawString(drOwner["LastName"] + ", " + drOwner["FirstName"], headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
            linesSoFarHeading++;
            ...
            g.DrawString(drOwner["Suburb"] + "", headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
            linesSoFarHeading++;
            g.DrawString("Cat Name: " + drCat["Name"] + "Visit ID: " + drVisit["VisitID"] + "Visit Date: " +  drVisit["VisitDate"], headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
            linesSoFarHeading++;
            linesSoFarHeading++;
            linesSoFarHeading++;
            ...
            g.DrawString(drVeterinarian["LastName"] + ", " + drVeterinarian["FirstName"], headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
            linesSoFarHeading++;

            visitTotal += Convert.ToDouble(drVeterinarian["Rate"]);

            DataRow[] drTreatment = drVisit.GetChildRows(DM.dtVisit.ChildRelations["VISIT_VISITTREATMENT"]);

            if (drTreatment.Length == 0)
            {
            ...
            {
                foreach (DataRow drVisitTreatment in drTreatment)
                {
                    ...
                    DataRow drTrtment = DM.dtTreatment.Rows[cmTreatment.Position];

                    double treatmentCost;

                    treatmentCost = Convert.ToInt32(drVisitTreatment["Quantity"]) *
                                    Convert.ToDouble(drTrtment["Cost"]);
                    visitTotal += treatmentCost;
                    txtInvoices.Text += treatmentText;
                    g.DrawString("tTreatment Description: " + drTrtment["Description"]
                                + "tQuantity: " + drVisitTreatment["Quantity"]
                                + "tTreatment Cost: " + Convert.ToString(treatmentCost), headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
                    linesSoFarHeading++;
                }
                linesSoFarHeading++;
                g.DrawString("Gross Due: " + Convert.ToString(visitTotal), headingFont, brush, leftMargin + headingLeftMargin,
                         topMargin + (linesSoFarHeading * textFont.Height));
                linesSoFarHeading++;
                linesSoFarHeading++;
                linesSoFarHeading++;
        ...
        private void btnDisplayReport_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow drVisit in DM.dtVisit.Rows)
            {
                ...
                if (visitStatus.Equals("Pending") == true)
                {
                    ...
                    // get the veterinarian record matching teh veterinarian ID from the visit record
                    int aVeterinarianID = Convert.ToInt32(drVisit["VeterinarianID"].ToString());
                    cmVeterinarian.Position = DM.veterinarianView.Find(aVeterinarianID);
                    DataRow drVeterinarian = DM.dtVeterinarian.Rows[cmVeterinarian.Position];
